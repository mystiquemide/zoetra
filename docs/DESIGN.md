...
Landing (/): hero: "Uptime you can slash." sub: one sentence, two CTAs (Live dashboard, GitHub). Three-step row (Register & stake → Heartbeat on-chain → Breach gets slashed) using Lucide `FileSignature`, `HeartPulse`, `Scissors`. Live proof strip pulling real StatsStrip numbers. Footer: GitHub repo, scan.bohr.life contract link, BOT Chain docs, @BOTChain_ai. All links real.
...
| 1:15 | Restart daemon, dot resumes, score climbs | "Honesty is recoverable. Fiction isn't. That's Zoetra: DePIN uptime you can slash" |
